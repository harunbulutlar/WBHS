...
for(var r=n[i],d=[],l=0;l<a.length;l++)t=a[l][1]?a[l][1](r[a[l][0]]):r[a[l][0]],d.push(' "'+a[l][0]+'": '+this._serialize_json_value(t));e.push("{"+d.join(",")+"}")}return"["+e.join(",\n")+"]"},e.toICal=function(t){var a="BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//dhtmlXScheduler//NONSGML v2.2//EN\nDESCRIPTION:",n="END:VCALENDAR",i=e.date.date_to_str("%Y%m%dT%H%i%s"),r=e.date.date_to_str("%Y%m%d"),d=[],l=this._get_serializable_data();for(var o in l){var s=l[o];d.push("BEGIN:VEVENT"),d.push(s._timed&&(s.start_date.getHours()||s.start_date.getMinutes())?"DTSTART:"+i(s.start_date):"DTSTART:"+r(s.start_date)),
